...
only the "should honor timeout" test surfaces explicitly in the log
(line 6676); the rest are condensed into the `•`/`S` bullet stream but are
tracked here per the failure-bucket count.
...
| `should honor timeout` | webhook.go:358 | FAIL | `should_honor_timeout` | mirrored, ignored (tracks failure) |
...
6. Webhook timeout — `should_honor_timeout` (the one surfaced in the captured e2e log at line 6676).

All six are `#[ignore]`d with a reason that points back here so a future
fix-pass can flip them on by removing the `ignore` and re-asserting.
