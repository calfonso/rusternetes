...
**2026-06-06:** The `known-green.txt` ratchet has been **retired**. The conformance canary no longer enforces a per-test allowlist; it now runs the full two-phase `[Conformance]` tag split (`scripts/conformance-tags-run.sh` — phase 1 parallel skipping `[Slow]`/`[Serial]`/`[Flaky]`, phase 2 the `[Serial]`/`[Slow]` subset single-threaded) as non-gating discovery/reporting. `ci/conformance/known-green.txt` and `scripts/conformance-canary-run.sh` were removed. Dated `known-green.txt` references in earlier entries are historical and describe the mechanism as it stood at the time.
